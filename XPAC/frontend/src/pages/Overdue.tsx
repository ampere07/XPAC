...
import { ChevronsLeft, ChevronLeft, ChevronRight ,ChevronsRight, X, Menu, Globe, Calendar, ChevronDown, RefreshCw, ArrowUp, ArrowDown, Columns3, Download, Filter } from 'lucide-react';
 ...
 *
 * 'plan' is a checklist whose options are derived from the loaded records, which avoids adding a
 * lookup endpoint just to filter a column the data already enumerates.
 */
const funnelColumns: FunnelColumn[] = [
  ...
  { key: 'overdue_date', label: 'Overdue Date', dataType: 'date' },
...
const OverduePage: React.FC = () => {
  ...
  const funnel = useFunnelFilter({
    storageKey: 'overdueFunnelFilters',
  ...
  return (
    <div className={`h-full flex flex-col md:flex-row overflow-hidden ${isDarkMode ? 'bg-gray-950' : 'bg-gray-50'
      ...
      <TableFunnelFilter
        {...funnel.panelProps}
        title="Overdue Filters"
        subtitle="Refine your overdue results"
      />
    </div>
  );
};
