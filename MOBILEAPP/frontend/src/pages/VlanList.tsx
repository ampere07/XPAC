...
const VlanList: React.FC = () => {
  ...
  // matching the web page this was ported from.
  ...
  // Auto-refresh every 15 minutes (silent). The web page hangs this off an idle
  // timer reset by mouse and keyboard activity; there is no equivalent signal
  // here, so it is a plain interval and the pull-to-refresh below covers the
  // "I want it now" case.
  useEffect(() => {
    const IDLE_TIME_LIMIT = 15 * 60 * 1000;
    const intervalId = setInterval(() => {
      loadVlans(true).catch((err) => console.error('Idle refresh failed:', err));
    }, IDLE_TIME_LIMIT);
  ...
  const executeDelete = async (vlan: Vlan) => {
    ...
    setDeletingItems((prev) => new Set(prev).add(vlan.id));
    showGlobalModal('loading', 'Deleting', `Removing VLAN "${vlan.value}"...`);

    try {
      const response = await apiClient.delete(`/vlans/${vlan.id}`);
      const data = response.data;
  ...
  const filteredVlans = vlans.filter((vlan) => {
    const matchesOrg = currentUserOrgId ? vlan.organization_id === currentUserOrgId : !vlan.organization_id;
  ...
  const paginatedVlans = filteredVlans.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  const renderListItem = ({ item: vlan }: { item: Vlan }) => (
    <TouchableOpacity
      onPress={() => handleEdit(vlan)}
      ...
      <View style={{ flex: 1, minWidth: 0, paddingRight: 16 }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginTop: 4 }}>
          <Text style={{ fontSize: 10, textTransform: 'uppercase', fontWeight: '500', color: '#9ca3af' }}>
            Created: {formatDate(vlan.created_at)}
          </Text>
          <Text style={{ fontSize: 10, textTransform: 'uppercase', fontWeight: '500', color: '#9ca3af' }}>
            Updated: {formatDate(vlan.updated_at)}
          </Text>
        </View>
      </View>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
        {actions.canEdit && (
          <TouchableOpacity onPress={() => handleEdit(vlan)} style={{ padding: 8, borderRadius: 6 }}>
            <Edit2 size={18} color="#4b5563" />
          </TouchableOpacity>
        )}
        {actions.canDelete && (
          <TouchableOpacity
            onPress={() => handleDelete(vlan)}
            disabled={deletingItems.has(vlan.id)}
            style={{ padding: 8, borderRadius: 6, opacity: deletingItems.has(vlan.id) ? 0.5 : 1 }}
          >
      ...
      >
        ...
        {actions.canCreate && (
          ...
          >
            ...
            {isTablet && <Text style={{ color: '#ffffff', fontSize: 12, fontWeight: '500' }}>Add VLAN</Text>}
          </TouchableOpacity>
        )}
        <TouchableOpacity
          onPress={() => loadVlans()}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={{ paddingVertical: 80, alignItems: 'center' }}>
              <Text style={{ color: '#6b7280' }}>No VLANs found</Text>
            </View>
          }
        />
      )}
