...
export const smartOltReconciliationService = {
  ...
  exportCsv: async (
    dataset: 'inventory' | 'alignment' | 'sn_alignment' | 'profile' | 'cleanup'
    ...
    // is fetched with expo-file-system, which also carries the auth headers.
    // Both spellings are sent because part of the chain to PHP strips
    // Authorization; see config/api.ts.
    const target = `${ExpoFileSystem.cacheDirectory}smartolt-${dataset}-${Date.now()}.csv`;
    ...
    try {
      ...
      const result = await ExpoFileSystem.downloadAsync(
        `${API_BASE_URL}${BASE}/export?dataset=${encodeURIComponent(dataset)}`,
        target,
        { headers }
      );
