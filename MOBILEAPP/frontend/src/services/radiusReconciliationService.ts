...
export const radiusReconciliationService = {
   ...
   */
  exportCsv: async (filter: string, serverId: string): Promise<{ uri: string } | { error: string }> => {
    // The web builds an object URL and clicks a hidden link. Neither exists on
    // this platform, and axios cannot reliably produce a Blob here, so the file
    // is fetched with expo-file-system instead — which also carries the auth
    // headers the endpoint needs. Both spellings are sent because part of the
    // chain between the phone and PHP strips Authorization; see config/api.ts.
    const name = `radius-reconciliation-${filter}-${Date.now()}.csv`;
    const target = `${ExpoFileSystem.cacheDirectory}${name}`;
    ...
    try {
      ...
      const result = await ExpoFileSystem.downloadAsync(
        `${API_BASE_URL}${BASE}/export?filter=${encodeURIComponent(filter)}&server_id=${encodeURIComponent(serverId)}`,
        target,
        { headers }
      );

      if (result.status >= 400) {
        return { error: `The export failed (HTTP ${result.status}).` };
      }

      return { uri: result.uri };
    } catch (error: any) {
      return { error: error?.message || 'The export failed.' };
    }
  },
};
