...
export const statusSliceService = {
  ...
  reset: async (moduleKey: string): Promise<void> => {
    ...
    try {
      // AsyncStorage rather than localStorage: same mirrored copy, this
      // platform's store. Awaited so a failure is caught by the same
      // handler that already wraps this cleanup.
      await AsyncStorage.removeItem(`user_pref_${preferenceKey(moduleKey)}`);
    } catch {
